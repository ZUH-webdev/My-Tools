...
import { convertHexToAll } from '../utils/convertHexToRgba';
...
const HexToRgba = () => {
    ...
    const handleConvert = () => {
        const data = convertHexToAll(hexInput);
    ...
    return (
        <div className="max-w-5xl mx-auto p-4 md:p-2 space-y-10 font-manrope mt-6">
            ...
            <div className="flex justify-center md:justify-end">
                <button
                    onClick={() => copyToClipboard(result.rgba, 'main-rgba')}
                    className={`group flex items-center gap-2 px-8 py-3 rounded-full border transition-all active:scale-95 font-bold ${copiedField === 'main-rgba'
                        ? 'bg-green-500 border-green-500 text-white'
                        : 'border-indigo-500 text-indigo-600 hover:bg-indigo-600 hover:text-white hover:border-indigo-600'
                        }`}
                >
                    {copiedField === 'main-rgba' ? (
                        <Check className="w-4 h-4 stroke-[3px]" />
                    ) : (
                        <Copy className="w-4 h-4 stroke-[3px]" />
                    )}
                    <span>
                        {copiedField === 'main-rgba' ? 'Copied!' : 'Copy RGBA Color'}
                    </span>
                </button>
            </div>
